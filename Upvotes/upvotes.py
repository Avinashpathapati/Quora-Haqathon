...
class TournamentTree:
	...
	def __init__(self, input, t):
		...
		self.array = [None] * self.offset
		self.array.extend([Node(value, i) for i,value in enumerate(input)])
		self.array.extend([None] * (self.offset - len(input)))
		self.type = t #true if non decreasing type, false if non increasing type
		self.__initParents()

	def check_combine(self,type, left, right):
		if type:
			#non decreasing
			return left <= right
		else:
	...
	def combine_nodes(self,leftChild, rightChild, type):
		...
		if self.check_combine(type, leftChild.last, rightChild.first):
			#combine
	...
	def __initParents(self):
		...
		while (left > 0):
			for i in range(left, right, 2):
				...
				self.array[i // 2] = self.combine_nodes(leftChild, rightChild, self.type)
			right = left
			left = left / 2
	...
	def query(self,i,lo, hi):
		...
		if(lo <= node.lo and hi >= node.hi):
			...
			return self.combine_nodes(self.query(2*i, lo, hi), self.query(2*i+1, lo, hi), self.type)
